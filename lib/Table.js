...
module.exports = class Table {
  ...
  buildTableReq(name, schema) {
    ...
    for (var globalIndexName of Object.keys(schema.indexes.global)) {
    ...
    for (var indexName of Object.keys(schema.indexes.local)) {
    ...
    var localSecIndexes = [];
    var index;
    for (var localSecIndexName of Object.keys(schema.indexes.local)) {
    ...
    for (var globalSecIndexName of Object.keys(schema.indexes.global)) {
...
var compareIndexes = (local, remote) => {
  const picked_fields = ['IndexName', 'KeySchema', 'Projection', 'ProvisionedThroughput'];
  ...
  var localIndexes = _.map(local.GlobalSecondaryIndexes, item => {
    return _.pick(item, picked_fields);
  });

  var remoteIndexes = _.map(remote.GlobalSecondaryIndexes, item => {
    return _.pick(item, picked_fields);
  });
