...
class Entity {
  ...
  put(options, next) { //TODO: FULL GUT!
    debug('put', this);
  ...
  delete(options, next) { //TODO: FULL GUT!
    options = options || {};
    if (typeof options === 'function') {
      next = options;
      options = {};
    ...
    const getDelete = {
      TableName: record.MODEL.table.name,
      Key: {}
    };
...
class Model {
  constructor(name, schema, options, base) {
    ...
    obj.prototype.save = obj.prototype.put;

    obj.name = obj.prototype.name = name;
    obj.table = obj.prototype.table = new Table(name, schema, options, base);
    obj.base = obj.prototype.base = base;
    obj.schema = obj.prototype.schema = schema;
    obj.options = obj.prototype.options = options;

    obj.get = tcWrapper(obj.get);
    obj.update = tcWrapper(obj.update);
    obj.delete = tcWrapper(obj.delete);
    obj.put = tcWrapper(obj.put);
    obj.prototype.save = obj.save = tcWrapper(obj.save);

    obj.query = tcWrapper(obj.query);
    obj.queryOne = tcWrapper(obj.queryOne);
    obj.scan = tcWrapper(obj.scan);

    obj.create = tcWrapper(obj.create);
    obj.batchGet = tcWrapper(obj.batchGet);
    obj.batchPut = tcWrapper(obj.batchPut);
    obj.batchDelete = tcWrapper(obj.batchDelete);
  ...
  delete(key, options, next) {
    ...
    const model1 = this;
    const model = new model1(key);//TODO: new Model?
  ...
  get(key, options, next) {
    ...
    function get() {
      ...
      model.base.ddb().getItem(getReq, function(err, data) {
        if (err) {
        ...
        const model = new model1(); //TODO: new Model?
  ...
  update(key, update, options, next) {
    ...
    function updateItem() {
      ...
      model.base.ddb().updateItem(updateReq, function(err, data) {
        if (err) {
          debug('Error returned by updateItem', err);
          return deferred.reject(err);
        }
  ...
  batchGet(keys, options, next) {
    ...
    const getReq = {};
    batchReq.RequestItems[this.name] = getReq;
    ...
    //const model = new model1(obj);//TODO: new Model?

    function batchGet() {
      debug('batchGetItem', batchReq);
      model.base.ddb().batchGetItem(batchReq, function(err, data) {
        if (err) {
        ...
        const models = data.Responses[model.name] ? data.Responses[model.name].map(toModel) : [];
        if (data.UnprocessedKeys[model.name]) {
          // convert unprocessed keys back to dynamoose format
          models.unprocessed = data.UnprocessedKeys[model.name].Keys.map(function(key) {
            const ret = {};
  ...
  batchPut(items, options, next) {
    ...
    const batchRequests = toBatchChunks(this.name, items, MAX_BATCH_WRITE_SIZE, item => {
      return {
        PutRequest: {
    ...
    const batchPut = function () {
      batchWriteItems(model1, batchRequests).then(function (result) {
    ...
    if (this.options.waitForActive) {
      this.table.waitForActive().then(batchPut);
    } else {
      batchPut();
    }
    return deferred.promise.nodeify(next);
  ...
  batchDelete(keys, options, next) {
    ...
    const batchRequests = toBatchChunks(this.name, keys, MAX_BATCH_WRITE_SIZE, key => {
      const key_element = {};
    ...
    const batchDelete = function() {
      batchWriteItems(model1, batchRequests).then(result => {
        deferred.resolve(result);
      }).fail(function (err) {
        deferred.reject(err);
      });
...
function batchWriteItems(model, batchRequests) {
  ...
  const batchList = batchRequests.map(function (batchReq) {
    ...
    model.base.ddb().batchWriteItem(batchReq, function (err, data) {
      if (err) {
