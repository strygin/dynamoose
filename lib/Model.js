...
function batchWriteItems(model, batchRequests) {
  ...
  const batchList = batchRequests.map((batchReq) => {
    const deferredBatch = Promise.defer();
  ...
  return Promise.all(batchList).then((resultList) => {
...
const tcWrapper = function(f) {
  return function() {
    try {
      ...
      return Promise.reject(err).suppressUnhandledRejections(); // TODO: handlers are attached asynchronously somewhere
    }
  };
};
