...
class Record {
  ...
  delete(options, next) {
    ...
    function deleteItem() {
      ...
      model.base.ddb().deleteItem(getDelete, (err, data) => {
        if (err) {
...
class Model {
  ...
  delete(key, options, next) {
    ...
    const record = new this(key);

    return record.delete(options, next);
  }
  ...
  get(key, options, next) {
    ...
    function get() {
      ...
      model.base.ddb().getItem(getReq, (err, data) => {
        if (err) {
        ...
        schema.parseDynamo(record, data.Item);

        debug('getItem parsed record', record);
  ...
  update(key, update, options, next) {
    ...
    function updateItem() {
      ...
      model.base.ddb().updateItem(updateReq, (err, data) => {
        if (err) {
          debug('Error returned by updateItem', err);
          return deferred.reject(err);
        }
        ...
        const record = new model();
        schema.parseDynamo(record, data.Attributes);

        debug('updateItem parsed record', record);
  ...
  batchGet(keys, options, next) {
    ...
    function batchGet() {
      ...
      model.base.ddb().batchGetItem(batchReq, (err, data) => {
        if (err) {
        ...
        function toModel(item) {
          const record = new model();
          schema.parseDynamo(record, item);

          debug('batchGet parsed model', record);
        ...
        if (data.UnprocessedKeys[model.name]) {
          ...
          models.unprocessed = data.UnprocessedKeys[model.name].Keys.map(key => {
            const ret = {};
  ...
  create(obj, options, next) {
    ...
    return record.save(options, next);
  }
