...
class Document {
  ...
  constructor(new_obj) {
    ...
    this.put = tcWrapper(this.put);
    //this.save = this.put;
    //this.delete = tcWrapper(this.put);
    // ['save', 'put', 'delete'].forEach(method => {
    //     this[method] = tcWrapper(this[method]);
    // });
  ...
  delete(options, next) {
    ...
    const getDelete = {
      TableName: document.MODEL.table.name,
      Key: {}
    };
      ...
      model.base.ddb().deleteItem(getDelete, (err, data) => {
        ...
        if (options.update && data.Attributes) {
          try {
            ...
            debug('deleteItem parsed model', model); // TODO: or this document?
          } catch (err) {
            return deferred.reject(err);
          }
        }
        deferred.resolve(model); // TODO: or this document?
      });
    }
    if (model.options.waitForActive) {
...
class Model {
  constructor(name, schema, options, base) {
    ...
    const obj = class {
      constructor(new_obj) {
        const document = new Document(new_obj);
    ...
    ['create', 'get', 'update', 'delete', // CRUD ;)
     ...
     'batchGet', 'batchPut', 'batchDelete'].forEach(method => {
        obj[method] = tcWrapper(obj[method]);
    });

    obj.prototype.save = obj.save =
        obj.prototype.put = obj.put = tcWrapper(obj.put);
    obj.prototype.delete = tcWrapper(obj.delete);
  ...
  delete(key, options, next) {
    ...
    const document = new this(key);

    return document.delete(options, next);
  }
