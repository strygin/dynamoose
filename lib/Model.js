...
class Model {
  constructor(name, schema, options, base) {
    ...
    debug('registering hooks');
    _.forEach(hooks, (hook, key) => {
      obj.prototype[key] = obj[key] = hook;
    });
  ...
  batchGet(keys, options, next) {
    ...
    function batchGet() {
      ...
      ThisModel.base.ddb().batchGetItem(batchReq, (err, data) => {
        ...
        const models = data.Responses[ThisModel.name] ? data.Responses[ThisModel.name].map(toModel) : [];
        if (data.UnprocessedKeys[ThisModel.name]) {
          // convert unprocessed keys back to dynamoose format
          models.unprocessed = data.UnprocessedKeys[ThisModel.name].Keys.map(key => {
            const ret = {};
  ...
  batchPut(items, options, next) {
    ...
    const batchRequests = toBatchChunks(this.name, items, MAX_BATCH_WRITE_SIZE, item => {
      return {
        PutRequest: {
          Item: ThisModel.schema.toDynamo(item)
        }
      };
    });
